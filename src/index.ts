...
interface Stats {
  perUrl: Record<string, { total: number; up: number; down: number; uptimePct: string }>;
  global: { totalMins: number; upMins: number; downMins: number; uptimePct: string };
  lastChecked: string;
  overallStatus: string;
}

function computeStats(logs: LogEntry[]): Stats {
  const urlMap: Record<string, { total: number; up: number; down: number }> = {};
  let globalUp = 0;
  let globalDown = 0;

  for (const log of logs) {
    if (!urlMap[log.url]) urlMap[log.url] = { total: 0, up: 0, down: 0 };
  ...
  for (const [url, d] of Object.entries(urlMap)) {
    perUrl[url] = {
      total: d.total, up: d.up, down: d.down,
      uptimePct: d.total > 0 ? ((d.up / d.total) * 100).toFixed(2) + '%' : 'N/A',
    };
  }
  ...
  return {
    perUrl,
    global: {
      totalMins, upMins: globalUp, downMins: globalDown,
...
async function checkUrl(browser: any, url: string): Promise<{ status: string; code: string; ms: number }> {
  ...
  try {
    ...
    return { status: resp && resp.status() >= 200 && resp.status() < 400 ? 'UP' : 'DOWN', code: httpCode, ms: elapsed };
  } catch (err: any) {
    const elapsed = Date.now() - start;
...
async function runCheck(urls: string[]) {
  if (running) return;
  running = true;
  console.log(`[${new Date().toLocaleString('en-US', { hour12: false })}] Checking ${urls.length} URLs...`);
  let browser: any;
  try {
...
function startServer() {
  ...
  app.get('/', (_req, res) => {
    ...
    if (existsSync(STATS_FILE)) {
      ...
      stats = {
        perUrl: {},
        global: { totalMins: 0, upMins: 0, downMins: 0, uptimePct: 'N/A' },
        lastChecked: 'Never',
        overallStatus: 'N/A',
      };
    }
    const urlRows = Object.entries(stats.perUrl).length > 0
      ? Object.entries(stats.perUrl).map(([url, s]) => {
          ...
          return `<tr>
            ...
            <td><span class="badge badge-${cls}">${s.uptimePct}</span></td>
          </tr>`;
        }).join('')
      : '<tr><td colspan="5" style="color:#64748b;text-align:center">No data yet</td></tr>';
...
<style>
  ...
  .url { word-break:break-all; font-size:0.9em; }
  .up { color:#4ade80; }
  .down { color:#f87171; }
...
<div class="stats">
  ...
  <div class="card"><div class="val" style="color:#4ade80">${stats.global.upMins}</div><div class="lbl">UP (mins)</div></div>
  <div class="card"><div class="val" style="color:#f87171">${stats.global.downMins}</div><div class="lbl">DOWN (mins)</div></div>
  <div class="card"><div class="val">${stats.global.uptimePct}</div><div class="lbl">Global Uptime</div></div>
</div>
<table>
<tr><th>URL</th><th>Checks</th><th>UP</th><th>DOWN</th><th>Uptime</th></tr>
${urlRows}
</table>
<div class="links">
